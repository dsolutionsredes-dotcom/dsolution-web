...
type Props = {
  ...
  editableLinks?: Array<PageElement | undefined>;
  ctaElement?: PageElement;
  visualEditingEnabled?: boolean;
  pageElements?: PageElement[];
};

function BrandLogo({ transparent, name, tagline, nameAttr, taglineAttr }: { transparent: boolean; name: string; tagline: string; nameAttr?: string; taglineAttr?: string }) {
  return (
    <span className="flex items-center gap-3">
      ...
      <span className="hidden leading-none sm:block">
        <span className={`block text-xl font-bold tracking-[-.03em] transition ${transparent ? 'text-white' : 'text-[#002147]'}`} data-directus={nameAttr}>{name}</span>
        <span className={`mt-1 block text-[.54rem] font-semibold uppercase tracking-[.12em] transition ${transparent ? 'text-white/70' : 'text-[#002147]/60'}`} data-directus={taglineAttr}>{tagline}</span>
      </span>
    </span>
  );
}

export default function Navbar({ links, ctaLabel, locale, onLocaleChange, transparentOnTop = false, editableLinks = [], ctaElement, visualEditingEnabled = false, pageElements = [] }: Props) {
  ...
  const serviceItem = (key: string) => pageElement(pageElements, 'global', locale, `service.${key}`);
  ...
  return <motion.header initial={{ y: -24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.55 }} className={headerClass}>
    <nav className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-3 md:px-8">
      <a href="/" className="flex shrink-0 items-center" aria-label="D-Solution Home">
        <BrandLogo transparent={transparent} name={elementText(brandName, 'D-Solution')} tagline={elementText(brandTagline, 'Audiovisual · Marketing Digital · Desarrollo Web')} nameAttr={directusAttr(visualEditingEnabled, 'page_elements', brandName?.id, 'text')} taglineAttr={directusAttr(visualEditingEnabled, 'page_elements', brandTagline?.id, 'text')} />
      </a>

      <div className="hidden items-center gap-7 md:flex">
        {links.map(([label, href], linkIndex) => isServices(label) ? <div key={label} ref={menuRef} className="relative flex items-center gap-1">
          <a href="/servicios" className={`text-sm font-semibold transition ${navText}`} data-directus={directusAttr(visualEditingEnabled, 'page_elements', editableLinks[linkIndex]?.id, ['text', 'link'])}>{label}</a>
          ...
          {servicesOpen && <div className="absolute left-[-1rem] top-9 w-72 rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_24px_55px_rgba(0,33,71,.17)]">
            {SERVICE_LINKS.map(service => { const editable = serviceItem(service.key); return <a key={service.key} href={editable?.link || service.href} className="block rounded-xl px-3 py-2 text-sm font-semibold text-[#002147]/80 transition hover:bg-[#F7F3EA] hover:text-[#D4AF37]" data-directus={directusAttr(visualEditingEnabled, 'page_elements', editable?.id, ['text', 'link'])}>{elementText(editable, locale === 'es' ? service.es : service.en)}</a>; })}
          </div>}
        </div> : <a key={`${label}-${href}`} href={href} className={`text-sm font-semibold transition ${navText}`} data-directus={directusAttr(visualEditingEnabled, 'page_elements', editableLinks[linkIndex]?.id, ['text', 'link'])}>{label}</a>)}
      </div>
      ...
      <div className="hidden items-center gap-3 md:flex">
        ...
        <a href={ctaElement?.link || '#contacto'} className={ctaClass} data-directus={directusAttr(visualEditingEnabled, 'page_elements', ctaElement?.id, ['text', 'link'])}>{ctaLabel}</a>
      </div>
    ...
    {open && <div className="border-t border-slate-200 bg-[rgba(247,243,234,.98)] px-5 pb-5 pt-3 md:hidden">
      <div className="grid gap-3">
        {links.map(([label, href], linkIndex) => isServices(label) ? <div key={label} className="grid gap-2"><a href="/servicios" onClick={() => setOpen(false)} className="text-sm font-semibold text-[#0B2340]/86" data-directus={directusAttr(visualEditingEnabled, 'page_elements', editableLinks[linkIndex]?.id, ['text', 'link'])}>{label}</a><div className="ml-2 grid gap-2 border-l border-[#D4AF37]/40 pl-3">{SERVICE_LINKS.map(s => { const editable = serviceItem(s.key); return <a key={s.key} href={editable?.link || s.href} onClick={() => setOpen(false)} className="text-sm text-[#002147]/75" data-directus={directusAttr(visualEditingEnabled, 'page_elements', editable?.id, ['text', 'link'])}>{elementText(editable, locale === 'es' ? s.es : s.en)}</a>; })}</div></div> : <a key={`${label}-${href}`} href={href} onClick={() => setOpen(false)} className="text-sm font-semibold text-[#0B2340]/86" data-directus={directusAttr(visualEditingEnabled, 'page_elements', editableLinks[linkIndex]?.id, ['text', 'link'])}>{label}</a>)}
        ...
        <a href={ctaElement?.link || '#contacto'} onClick={() => setOpen(false)} className="mt-2 inline-flex h-[52px] w-full items-center justify-center rounded-full bg-[#D4AF37] px-5 text-sm font-bold text-[#002147]" data-directus={directusAttr(visualEditingEnabled, 'page_elements', ctaElement?.id, ['text', 'link'])}>{ctaLabel}</a>
      </div>
    </div>}
  </motion.header>;
}
